...
sudo cp autorandr/bash_completion/autorandr /etc/bash_completion.d/

# Setup the script to run specific events occur power/sleep/dock/undock
sudo cp autorandr/pm-utils/40autorandr /usr/lib/pm-utils/sleep.d/
sudo cp autorandr/pm-utils/40autorandr /usr/lib/pm-utils/power.d/
sudo cp autorandr/udev/40-monitor-hotplug.rules /etc/udev/rules.d/
